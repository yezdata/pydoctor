...
def compute_loss(
    ...
    is_code = torch.cumsum(start_mask, dim=-1) > 0

    shift_targets = torch.where(is_code, shift_targets, -100)
...
def evaluate(
    ...
    with torch.no_grad():
        for batch in dataloader:
            ...
            loss = compute_loss(criterion, outputs, batch["input_ids"], start_token_ids)
...
def main(
    ...
    spec_tokens = tokenizer.encode(tokenizer_config.spec_tokens.model_dump().values())
    ...
    for epoch in range(train_config.num_epochs):
        ...
        for step, batch in tqdm(
            ...
            with accelerator.accumulate(model):
                ...
                loss = compute_loss(criterion, outputs, batch["input_ids"], spec_tokens)
