...
def main():
    ...
    tokenizer_cfg = TokenizerConfig(name="google/gemma-3-270m")
    print(f"Tokenizer: {tokenizer_cfg.name}")

    tokenizer = get_finetune_tokenizer(
        tokenizer_cfg,
    )

    train_config = FinetuneConfig(tokenized_ds_dir="TEST-gemma/tokenized_ds")  # type:ignore
