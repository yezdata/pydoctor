    ...
    for batch_file in batches_dir.glob("batch_*.json"):
        with open(batch_file, "r", encoding="utf-8") as f:
            ...
            for k, raw_text in b_results.items():
                ...
                if len(parts) == 3:
                    code, docstring = parts[0].strip(), parts[2].replace("<|endoftext|>", "").strip()
                    final_output = (
...
tokenized_ds.save_to_disk("data/tokenized_synthetic_the_stack_libs_instruct_smollm2_1_7B_instruct")